...
class Viterbi(object):
    ...
    def decode(self, log_frame_probs):
        ...
        #print (log_frame_probs.shape,frame_scores.shape)
    ...
    def decode_frame(self, t, old_hyp, frame_scores):
        ...
        for key, hyp in old_hyp.items():
            ...
            for new_label in self.grammar.possible_successors(context):
                ...
                score = hyp.score + self.frame_score(frame_scores, t, label) + self.grammar.score(context, new_label) + 1 * self.length_model.score(length, label) 
    ...
    def finalize_decoding(self, old_hyp):
        ...
        for key, hyp in old_hyp.items():
            context, label, length = key[0:-2], key[-2], key[-1]
            context = context + (label,)
            score = hyp.score + self.grammar.score(context, self.grammar.end_symbol()) + 1 * self.length_model.score(length, label) 
